...
OUTPUT_VIDEO = "output_pov_lyric_hollywood_title_sequence_smooth_handoff.mp4"

FONT_PATH = "fonts/Anton-Regular.ttf"
TEMP_VIDEO_NO_AUDIO = "_temp_pov_lyric_hollywood_title_sequence_smooth_handoff.mp4"
...
PROBLEMS_COUNTER_DURATION = 23.80
...
def build_auto_problems_counter_overlay(
    ...
    """
    Code-driven counter with random loading speed and a polished final ending:
    1) Random 100% -> 0% loading
    2) Short 0% hold with PROBLEMS
    3) Only PROBLEMS stays for 2-3s as the final emphasis
    4) PROBLEMS dissolves into beautiful fragments / particles
    """
    ...
    if is_zero_hold:
        zero_hold_p = clamp(post_zero_elapsed / max(0.001, PROBLEMS_ZERO_HOLD_SECONDS), 0.0, 1.0)
        ...
        paste_layer_rgba(overlay, percent_layer, anchor_x, anchor_y)
        ...
        # light sweep starts from zero-hold and continues into solo stage
        ...
        sweep = make_handoff_light_sweep(
            ...
            x=anchor_x,
            y=problems_y,
            text_w=max(260, int(percent_font_size * 4.2)),
            text_h=max(60, int(problems_font_size * 1.25)),
            progress=sweep_p,
        )
    ...
    if is_final_solo:
        ...
        # Continue the same PROBLEMS layer. No alpha reset.
        ...
        # Continue light sweep only at the beginning of final solo.
        sweep_p = clamp(post_zero_elapsed / max(0.001, PROBLEMS_HANDOFF_REBUILD_SECONDS), 0.0, 1.0)
        sweep = make_handoff_light_sweep(
            frame_w=frame_w,
            frame_h=frame_h,
    ...
    if is_final_dissolve:
        ...
        # Brief bright glow at the start of the dissolve, then fades.
