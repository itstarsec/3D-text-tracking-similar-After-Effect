...
OUTPUT_VIDEO = "output_pov_lyric_problems_letter_reveal_shatter.mp4"

FONT_PATH = "fonts/Anton-Regular.ttf"
TEMP_VIDEO_NO_AUDIO = "_temp_pov_lyric_problems_letter_reveal_shatter.mp4"
...
PROBLEMS_FINAL_SOLO_HOLD_SECONDS = 2.10
...
# Letter-by-letter final reveal:
# Sau loading xong, P R O B L E M S sẽ hiện dần từ mờ -> rõ,
# lần lượt từ P đến S, giữ full word 0.75s rồi mới phân rã.
PROBLEMS_LETTER_REVEAL_SECONDS = 1.35
PROBLEMS_FINAL_HOLD_BEFORE_SHATTER = 0.75
PROBLEMS_LETTER_REVEAL_STAGGER = 0.085
PROBLEMS_LETTER_MIN_ALPHA = 0.10
PROBLEMS_LETTER_POP_SCALE = 1.055
PROBLEMS_LETTER_GLOW_ALPHA = 0.18
...
def get_random_counter_state(elapsed: float) -> Optional[Tuple[int, int, float, float]]:
    ...
    total_visible = (
        ...
        + PROBLEMS_LETTER_REVEAL_SECONDS
        + PROBLEMS_FINAL_HOLD_BEFORE_SHATTER
...
def render_problems_letter_reveal_layer(
    frame_w: int,
    frame_h: int,
    text: str,
    font,
    ...
    for ch in letters:
        ...
        letter_sizes.append((max(1, bbox[2] - bbox[0]), max(1, bbox[3] - bbox[1])))

    tracking = max(0, int(letter_sizes[0][0] * 0.035))
    cursor_x = x

    for i, ch in enumerate(letters):
        lw, lh = letter_sizes[i]
        letter_start = i * PROBLEMS_LETTER_REVEAL_STAGGER
        available = max(0.001, 1.0 - letter_start)
        lp = clamp((reveal_progress - letter_start) / available, 0.0, 1.0)
        lp = smoothstep(0.0, 0.58, lp)

        alpha = base_alpha * lerp(PROBLEMS_LETTER_MIN_ALPHA, 1.0, lp)
        pop = 1.0 + (PROBLEMS_LETTER_POP_SCALE - 1.0) * (1.0 - lp) * smoothstep(0.0, 0.35, reveal_progress)
        scale = base_scale * pop
        y_offset = int(round(lerp(5.0, 0.0, lp)))

        layer = render_single_text_layer(
            canvas_w=max(80, lw * 4),
            canvas_h=max(80, lh * 4),
            text=ch,
            font=font,
            fill_rgb=fill_rgb,
            alpha=alpha,
        ...
        paste_x = int(round(cursor_x - ((scale - 1.0) * lw) / 2.0))
        paste_y = int(round(y + y_offset - ((scale - 1.0) * lh) / 2.0))
        paste_layer_rgba(overlay, layer, paste_x, paste_y)
        cursor_x += lw + tracking

    return overlay


def final_stage_progress(post_zero_elapsed: float) -> Tuple[float, float, bool]:
    reveal_start = PROBLEMS_ZERO_HOLD_SECONDS
    reveal_end = reveal_start + PROBLEMS_LETTER_REVEAL_SECONDS
    hold_end = reveal_end + PROBLEMS_FINAL_HOLD_BEFORE_SHATTER

    reveal_progress = clamp(
        (post_zero_elapsed - reveal_start) / max(0.001, PROBLEMS_LETTER_REVEAL_SECONDS),
        0.0,
        1.0,
    )
    hold_progress = clamp(
        (post_zero_elapsed - reveal_end) / max(0.001, PROBLEMS_FINAL_HOLD_BEFORE_SHATTER),
        0.0,
        1.0,
    )
    return reveal_progress, hold_progress, post_zero_elapsed >= hold_end
...
def build_auto_problems_counter_overlay(
    ...
    """
    Counter + final PROBLEMS letter reveal:
    - numbers have no % and are centered above PROBLEMS
    - after loading reaches 0, 0 fades out
    - PROBLEMS appears letter-by-letter P -> S
    - full word holds 0.75s
    - then Hollywood shard/shatter dissolve starts
    """
    ...
    stage_letter_reveal_end = stage_zero_hold_end + PROBLEMS_LETTER_REVEAL_SECONDS
    stage_final_hold_end = stage_letter_reveal_end + PROBLEMS_FINAL_HOLD_BEFORE_SHATTER
    ...
    if is_zero_hold:
        ...
        problems_alpha = global_alpha * PROBLEMS_ZERO_HANDOFF_ALPHA
        problems_layer = render_fixed_text_layer(
            frame_w=frame_w,
            frame_h=frame_h,
            text="PROBLEMS",
    ...
    if is_final_solo:
        reveal_p, hold_p, _ = final_stage_progress(post_zero_elapsed)

        pulse = 1.0 + math.sin(hold_p * math.pi * 2.0) * (PROBLEMS_FINAL_PULSE_SCALE - 1.0) * max(0.0, hold_p)
        pulse_alpha = 1.0 + math.sin(hold_p * math.pi * 2.0) * PROBLEMS_FINAL_PULSE_ALPHA * max(0.0, hold_p)
        pulse_x = int(round(anchor_x - (problems_text_w * (pulse - 1.0)) / 2.0))

        problems_core = render_problems_letter_reveal_layer(
            frame_w=frame_w,
            frame_h=frame_h,
            text="PROBLEMS",
            font=problems_font,
            fill_rgb=fill_rgb,
            base_alpha=global_alpha * clamp(pulse_alpha, 0.0, 1.15),
            x=pulse_x,
            y=problems_y,
            reveal_progress=reveal_p,
            base_scale=pulse,
        )
        ...
        glow_alpha = PROBLEMS_LETTER_GLOW_ALPHA * (0.35 + 0.65 * smoothstep(0.0, 1.0, reveal_p))
        glow = make_glow_from_rgba(problems_core, blur_radius=21, alpha_mul=glow_alpha)
    ...
    if is_final_dissolve:
        ...
        base_problems = render_problems_letter_reveal_layer(
            frame_w=frame_w,
            frame_h=frame_h,
            text="PROBLEMS",
            font=problems_font,
            ...
            y=problems_y,
            reveal_progress=1.0,
            base_scale=1.0 + 0.02 * smoothstep(0.0, 1.0, dissolve_p),
        )
